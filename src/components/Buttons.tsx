...
const Buttons = () => {
  ...
  return (
    <section className="d-flex justify-content-center gap-2 mt-5" id="menu">
      <Button  style={stateCategory === 'popular' ? {background: "#DC3545",color:'white'}: {}} variant="outline-danger" onClick={()=>dispatch(getStateCategory('popular'))}>POPULAR</Button>
      {categories.map((cate)=>(<Button key={cate.id} variant="outline-danger" style={stateCategory === `${cate.name}` ? {background: "#DC3545",color:'white'}: {}} onClick={()=>dispatch(getStateCategory(`${cate.name}`))}>{cate.name.toLocaleUpperCase()}</Button>))}
    </section>
  );
};
